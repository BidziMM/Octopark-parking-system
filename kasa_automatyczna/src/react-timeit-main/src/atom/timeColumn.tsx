...
const TimeColumn = forwardRef(({ start, end, setValue, value, exclude, notShowExclude }: TimeColumn, ref) => {
  ...
  const addH = (v:number) => {
    let prev = slecetorMove;
    if(prev + v > end){
      return setSlecetorMove((prev+v) - end-1)
    }
    if (prev !== end) {
      if (exclude?.includes(prev + v)) {
        while (exclude?.includes(prev + v)) {
          if (prev + v > end) {
            return setSlecetorMove(start);
          }
          prev = prev + v;
          setSlecetorMove(prev + v);
        }
      } else {
        return setSlecetorMove(prev + v);
      }
    } else {
      return setSlecetorMove(start);
    }
